...
| Play-button glow (fixed 2026-09-07) | The pulse ring on the home Play and win-screen Next buttons used `box-shadow: 0 0 0 10px` — zero blur, so a hard-edged 10px band rather than a glow. It passed unnoticed over the home screen's art but read as a grey-green slab around the button on the win dialog's flat purple. Now `0 0 18px 2px`; the blur rasterizes once and only transform/opacity animate, so the per-frame cost is unchanged. |
| Win-screen star arc (fixed 2026-09-07) | The three stars are meant to arc, middle one largest and highest. `.stars--arc i:nth-child(2)` was enlarging the **left** star instead: `.win__burst` is prepended to the row, so despite being `position: absolute` it still occupies child slot 1. Now `:nth-of-type(2)`, which counts only the `<i>` stars. Worth remembering wherever a decorative element shares a row with content. |
